...
const ChapterDashboard = () => {
    ...
    const handleDelete = async (id, type) => {
        ...
        try {
            const endpoint = type === 'chapter' ? `/chapter/delete/${id}` : `/material/delete/${id}`;
                            ...
                            >
                                ...
                                <div className="px-4 flex justify-between text-white hover:text-teal-400 w-full">
                                    <div>
                                        {item.types === 'chapter' ? (
                                            <Link to={`/exercises/${item._id}`}>
                                                <h3 className="font-semibold text-lg mb-1 hover:underline">{item.chapterName}</h3>
                                            </Link>
                                        ) : (
                                            <div>
                                    ...
                                    <div className="flex items-center space-x-4">
                                        <Link
                                            to={item.types === 'chapter' ? `/editchapter/${item._id}` : `/editmaterial/${item._id}`}
                                            className="text-white hover:text-yellow-500"
                                            title={`Edit ${item.types === 'chapter' ? 'Chapter' : 'Material'}`}
                                        >
                                            <FaEdit />
                                        </Link>
                                        <button
                                            ...
                                            title={`Delete ${item.types === 'chapter' ? 'Chapter' : 'Material'}`}
                                        >
                                            <FaTrash />
                                        </button>
                                    </div>
            ...
            <div className="fixed bottom-6 right-6 z-50" ref={dropdownRef}>
                ...
                {showAddOptions && (
                    <div className="absolute bottom-16 right-0 bg-gray-700 rounded-lg shadow-lg py-2 w-48">
                        <Link
                            to={`/addmaterial/subject/${subjectId}`}
